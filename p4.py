...
def estimate_initial_RT(E):
    ...
    Z = [[0,1,0],[-1,0,0],[0,0,0]]
    W = [[0,-1,0],[1,0,0],[0,0,1]]
    Q1 = np.matmul(U,np.matmul(np.transpose(W),Vh))
    Q2 = np.matmul(U,np.matmul(W,Vh))
    q1=np.linalg.det(Q1) 
    q2=np.linalg.det(Q2) 
    Q1 = q1*Q1
    Q2 = q2*Q2
    ...
    RT = np.zeros((4,3,4))
    RT[0,:,:] = np.concatenate((Q1,np.transpose(K1)),axis=1)
    RT[1,:,:] = np.concatenate((Q2,np.transpose(K1)),axis=1)
    RT[2,:,:] = np.concatenate((Q1,np.transpose(K2)),axis=1)
    RT[3,:,:] = np.concatenate((Q2,np.transpose(K2)),axis=1)
    return RT 
...
def linear_estimate_3d_point(image_points, camera_matrices):
    # TODO: Implement this method!
    rows,columns = image_points.shape
    A=np.zeros((2*rows,4))
    for i in range(rows):
...
def reprojection_error(point_3d, image_points, camera_matrices):
    # TODO: Implement this method!
    #print(camera_matrices.shape, point_3d.shape)
    
    point_3d_homogenous = [point_3d[0],point_3d[1],point_3d[2],1]
    
    
    
    y = np.matmul(camera_matrices, point_3d_homogenous)
    cam,row,column = camera_matrices.shape
    for i in range(cam): 
        y[i] = y[i]/y[i][2]
    y= y[:,:-1]
    error = y- image_points
    error = np.reshape(error,4)
    return(error)
    raise Exception('Not Implemented Error')
...
def jacobian(point_3d, camera_matrices):
    # TODO: Implement this method!
    cam, rows,columns= camera_matrices.shape
    jacobi = np.zeros((2*cam, 3))
    for i in range(cam):
        for j in range(3):
            jacobi[2*i][j] = camera_matrices[i][0][j]/camera_matrices[i][0][2]
            jacobi[2*i+1][j] = camera_matrices[i][1][j]/camera_matrices[i][0][2]
    print(jacobi)
    return 0
    raise Exception('Not Implemented Error')
